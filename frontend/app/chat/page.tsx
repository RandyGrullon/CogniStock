...
import { 
  Send, Bot, User, Sparkles, BrainCircuit, AlertCircle, 
  RefreshCw, Clock, Copy, Check, Wallet, BarChart3,
...
export default function AIChatPage() {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: "⚠️ ERROR: Se ha perdido la conexión con el núcleo de CogniStock.", timestamp: 'ERROR' }]);
    } finally {
      setIsTyping(false);
    }
  };
            ...
            {messages.length === 0 ? (
                ...
                >
                    ...
                    <div className="space-y-3 max-w-lg">
                        <h2 className="text-3xl font-black text-white tracking-tighter text-balance">Neural Interface v2.1</h2>
                        <p className="text-zinc-500 text-sm leading-relaxed font-medium px-6">
                            Bienvenido al núcleo de <span className="text-blue-400 font-bold">CogniStock</span>. 
                            He sincronizado tu wallet y lecciones pasadas. ¿Qué estrategia analizamos hoy?
                        </p>
                    </div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex justify-start">
                    <div className="flex items-start gap-4">
                        ...
                        <div className="bg-white/5 border border-white/10 p-6 rounded-3xl rounded-tl-none flex items-center gap-2 backdrop-blur-xl">
                            ...
                            <span className="text-[10px] font-mono text-blue-500 uppercase tracking-widest font-bold">Procesando Datos...</span>
                        </div>
                    </div>
                </motion.div>
            )}
          ...
          <form onSubmit={sendMessage} className="relative max-w-4xl mx-auto">
            <div className="relative group">
                ...
                <input 
                    ...
                    placeholder="Consulta al núcleo neuronal de CogniStock..."
        ...
        <motion.div initial={{ opacity: 0, y: 20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex gap-5 max-w-[90%] md:max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className="relative flex-shrink-0 mt-1">
                    ...
                    <div className={`relative w-11 h-11 rounded-[1.2rem] flex items-center justify-center border ${isUser ? 'bg-zinc-900 border-white/10 text-zinc-400' : 'bg-blue-600 border-blue-400/50 text-white shadow-inner shadow-blue-400/20'}`}>
