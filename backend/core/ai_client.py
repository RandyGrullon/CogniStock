...
class AIClient:
    ...
    def generate_daily_intentions(self, portfolio_status: Dict[str, Any], market_context: str) -> Dict[str, Any]:
        prompt = f"""
        Hoy abre el mercado. Como analista jefe, define tu estrategia y objetivos para el día basándote en tu portafolio y el contexto.
        ...
        {{
            "intencion": "Resumen de lo que planeas hacer hoy (ej: reducir exposición, buscar entradas en tech, etc.)",
            "objetivos": "Lista de objetivos específicos para hoy"
        }}
        """
        chat_completion = self.client.chat.completions.create(
            messages=[
    ...
    def generate_daily_review(self, intentions: Dict[str, Any], results: Dict[str, Any], market_context: str) -> Dict[str, Any]:
        prompt = f"""
        El mercado ha cerrado. Analiza si cumpliste tus objetivos de hoy.

        INTENCIONES DE ESTA MAÑANA:
        {intentions.get('intencion', 'N/A')}
        OBJETIVOS: {intentions.get('objetivos', 'N/A')}
